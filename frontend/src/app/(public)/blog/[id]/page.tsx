...
async function getBlog(id: string) {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/blogs/${id}`, { next: { revalidate: 3600 } });
...
export default async function BlogDetailPage({ params }: { params: { id: string } }) {
  const blog = await getBlog(params.id);
  if (!blog) return <div style={{ textAlign: 'center', padding: 80, color: 'var(--muted)' }}>Blog post not found.</div>;

  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '60px 24px', position: 'relative', zIndex: 1 }}>
      <a href="/blog" style={{ fontSize: 13, color: 'var(--muted)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 32 }}>
        ← Back to Blog
      </a>

      <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: 'var(--cyan)', marginBottom: 12 }}>{blog.category}</p>
      <h1 style={{
        fontSize: 'clamp(26px,4vw,40px)', fontWeight: 800, letterSpacing: -1.5,
